...
{
    //instance vars
    private ArrayList<T> _heap; //underlying container is array of Integers
	...
	while( addValPos > 0 ) { //potentially swap until reach root
	    ...
	    if ( ((Comparable)addVal).compareTo(_heap.get(parentPos)) < 0 ) {//addVal < parent
	...
	while( pos < _heap.size() ) {
	    ...
	    else if ( ((Comparable)foo).compareTo( _heap.get(minChildPos) ) <= 0 ) 
		break;
	...
	else if ( ((Comparable)_heap.get(lc)).compareTo(_heap.get(rc)) < 0 )
	    retVal = lc;
	else
	    retVal = rc;
	return retVal;
    ...
    {
	ALHeap<Integer> pile = new ALHeap<Integer>();
